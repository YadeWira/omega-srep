...
char *program_version     = "Omega SREP 1.0a-beta.4", *program_date = "April 27, 2026";
  ...
  *next_filename = NULL;
  ...
  if (filenames[1]==NULL) {
    printf (         "%s: %s\n"
                     "%s    homepage: %s\n"
                     "\n"
                     "Usage: osrep [options] infile [outfile]\n"
                     "   infile/outfile can be specified as \"-\" for stdin/stdout\n"
                     "   \"osrep [options] somefile\" compresses data from somefile to somefile.osr\n"
                     "   \"osrep [options] somefile.osr\" decompresses data back to somefile\n"
                     "   \"osrep [options]\" compresses and \"osrep -d [options]\" decompresses data from stdin to stdout\n"
                     "\n"
                     "Options are:\n"
                     ...
                     "   -i: print info about compressed file: osrep -i datafile.osr\n"
                     ...
                     "\n"
                     "Omega SREP additions (F5 / F6):\n"
                     "   -dup: enable FA-style dedup pre-pass (CDC + content hash + ref table)\n"
                     "         streaming end-to-end; native flag on osrep; auto-detected on decompress\n"
                     "         incompatible with -m0; use -m3/-m4/-m5 for best results\n"
                     "   --dup-paranoid: byte-compare on every dedup hit (eliminates the 64-bit hash\n"
                     "                   collision window for archival workloads; one disk seek per hit)\n"
                     "   --chunk-avg=N / --chunk-min=N / --chunk-max=N / --chunk-buf=N\n"
                     "                   CDC tunables for -dup (defaults match FA: 4k/1k/16k/8m bytes)\n"
                     "   --chunk-hash=NAME: CDC boundary hash for -dup: fnv (default) or gear.\n"
                     "                   gear = Gear-hash CDC with an implicit ~64-byte content\n"
                     "                   window (FastCDC-style normalized chunking); finds\n"
                     "                   non-buffer-aligned duplicates fnv's unwindowed rolling\n"
                     "                   hash misses. Opt-in; does not change the .dupref format.\n"
...
// F5.3b: defines the program's int main, wrapping srep_main with the
// -dup pre/post-process pipeline. Must come AFTER srep_main is fully
// defined so the unqualified call inside dup_wrapper.cpp resolves.
#include "dup_wrapper.cpp"
