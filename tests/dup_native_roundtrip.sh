...
for method in -m1 -m2 -m3 -m4 -m5; do
    ...
    if ! "$OSREP" -dup "$method" "$in" "$archive" >/dev/null 2>&1; then
        echo "FAIL  $method  compress (native -dup) failed"
        failed=$((failed + 1)); continue
    fi
    if ! "$OSREP" -d "$archive" "$restored" >/dev/null 2>&1; then
...
if "$OSREP" -dup -m0 "$TMP/m0_in.bin" "$TMP/m0.osr" >/dev/null 2>&1; then
...
"$OSREP" -m4 "$TMP/xa.body" "$TMP/xa.body.osr" >/dev/null 2>&1
...
if "$OSREP" -d "$TMP/xa.archive" "$TMP/xa.restored" >/dev/null 2>&1 && \
...
"$OSREP" -d "$TMP/xb.body.osr" "$TMP/xb.body.dec" >/dev/null 2>&1
...
gen_input "$xs_in"
"$OSREP" --seed=12345 -m4 "$xs_in" "$TMP/seed_a.osr" >/dev/null 2>&1
"$OSREP" --seed=12345 -m4 "$xs_in" "$TMP/seed_b.osr" >/dev/null 2>&1
"$OSREP" --seed=99999 -m4 "$xs_in" "$TMP/seed_c.osr" >/dev/null 2>&1
"$OSREP"             -m4 "$xs_in" "$TMP/seed_d.osr" >/dev/null 2>&1
"$OSREP"             -m4 "$xs_in" "$TMP/seed_e.osr" >/dev/null 2>&1
seed_ok=1
...
"$OSREP" -d "$TMP/seed_a.osr" "$TMP/seed_dec.bin" >/dev/null 2>&1
...
if "$OSREP" -dup --dup-paranoid -m4 "$xp_in" "$xp_archive" >/dev/null 2>&1 && \
   "$OSREP" -d "$xp_archive" "$xp_restored" >/dev/null 2>&1 && \
...
"$OSREP" -dup -m4                "$xq_in" "$xq_normal"   >/dev/null 2>&1
"$OSREP" -dup --dup-paranoid -m4 "$xq_in" "$xq_paranoid" >/dev/null 2>&1
"$OSREP" -d "$xq_normal"   "$xq_normal_dec"   >/dev/null 2>&1
"$OSREP" -d "$xq_paranoid" "$xq_paranoid_dec" >/dev/null 2>&1
...
edge_case() {
    ...
    if ! "$OSREP" -dup -m4 "$input_path" "$archive" >/dev/null 2>&1; then
...
: > "$TMP/edge_empty.bin"
plain_rc=0
"$OSREP" -m4 "$TMP/edge_empty.bin" "$TMP/edge_empty_plain.osr" >/dev/null 2>&1 || plain_rc=$?
dup_rc=0
"$OSREP" -dup -m4 "$TMP/edge_empty.bin" "$TMP/edge_empty_dup.osr" >/dev/null 2>&1 || dup_rc=$?
