...
for m in m0 m1 m2 m3 m4 m5; do
    "$OSREP" -$m "$TMP/in.bin" "$TMP/$m.osr" >/dev/null 2>&1 || { echo "FAIL -$m compress"; fail=$((fail+1)); continue; }
    # Low memory + tiny VM block: exercises save_to_disk/restore_from_disk.
    rm -f "$TMP/$m.vm"
    if out=$("$OSREP" -d -mem8mb -vmblock=64kb -vmfile="$TMP/$m.vm" "$TMP/$m.osr" "$TMP/$m.out" 2>&1); then
...
if "$OSREP" -d -vmblock=4mb "$TMP/m3.osr" "$TMP/x.out" 2>&1 | grep -qi "invalid option"; then
    echo "FAIL: -vmblock= rejected"; fail=$((fail+1))
else
    pass=$((pass+1))
fi
if "$OSREP" -d -vmfile="$TMP/y.vm" "$TMP/m3.osr" "$TMP/y.out" 2>&1 | grep -qi "invalid option"; then
